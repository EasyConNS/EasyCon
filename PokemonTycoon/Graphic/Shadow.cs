...
namespace PokemonTycoon.Graphic
{
    public class Shadow
    {
        ...
        public static string GetString(int id, char variance)
        {
            var idstr = id == 0 ? "???" : $"{id:000}";
            var v = variance;
            if (v == '\0' && _genderdiff.Contains(id))
                v = 'm';
            ...
            return $"#{idstr} {prefix}{Pokemon.Names[id]}{postfix}";
        }

        public override string ToString()
        {
            return GetString(ID, Variance);
        }
    }
}
