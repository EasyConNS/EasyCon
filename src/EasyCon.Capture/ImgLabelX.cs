...
public class ImageLabelX
{
    ...
    /// 目标区域坐标和尺寸
    /// </summary>
    private Rectangle TargetRegion { get; set; }
    /// <summary>
    /// 搜索区域坐标和尺寸
    /// </summary>
    private Rectangle SearchRegion { get; set; }

    /// <summary>
    /// 标签数据
    /// </summary>
    private string TargetData { get; set; } = string.Empty;

    /// <summary>
    /// 处理方式（根据数据类型自动选择）
    /// </summary>
    private SearchMethod ProcessingMethod { get; set; } = SearchMethod.SqDiffNormed;
